...
class GameController {
    ...
    async getGame(req, res, next) {
        ...
        if (gameId === req.player.gameId) {
            ...
            if (!responseGame[0].idPlayer2) {
                ...
                if (!idPlayer2) { return res.status(400).send('Id Player2 is not in body') }
            ...
            if (responseGame[0].points.w === 12) {
                ...
                return res.status(200).send({
                    ...
                    message: `Game Over, ${responseGame[0].idPlayer1} won`,
                ...
                return res.status(200).send({
                    ...
                    message: `Game Over, ${responseGame[0].idPlayer2} won`,
                    scoreboard: `White ${responseGame[0].points.w}:${responseGame[0].points.b} Black`
    ...
    async getGameToMove(item) {

        const { gameId } = item.query
        const { idPlayer2 } = item.body

        if (gameId === item.player.gameId) {

            var responseGame = await new Database('src/db/games.json').read(gameId)

            if (responseGame==='Id not found') { return responseGame }

            if (!responseGame[0].idPlayer2) {

                await new PlayerController().updateGame(gameId, idPlayer2)
